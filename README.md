...
打开dome01里的main.js `` import Vue from 'vue' import vueRouter from "vue-router" //引进路由 import App from './App.vue'
Vue.use(vueRouter) //使用路由 new Vue({ el: '#app', render: h => h(App) })

`` 9. 在scr里新建components文件。在components文件里新建两个组件文件Mains.vue和Deail.vue 10. 在App.vue里写：
